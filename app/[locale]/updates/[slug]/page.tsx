...
export default function UpdateDetailPage() {
  ...
  const getSourceIcon = () => {
    switch (update.sourceType) {
      case "partner": return <Zap size={16} className="text-[#006db1]" />;
      case "industry": return <Factory size={16} className="text-[#0087cb]" />;
      case "internal": return <Rss size={16} className="text-[#006db1]" />;
    }
  };

  return (
    <div className="bg-black text-white font-mono selection:bg-[#006db1] selection:text-black min-h-screen pb-24">
      
      {/* --- NAVIGATION --- */}
      <nav className="fixed top-0 w-full z-50 bg-black/80 backdrop-blur-md border-b border-white/5 px-6 py-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <Link href="/updates" className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-zinc-500 hover:text-[#006db1] transition-colors">
          ...
          <div className="flex gap-6">
            ...
            <span className="px-3 py-1 bg-[#006db1] text-black text-[9px] font-[1000] uppercase tracking-tighter">
      ...
      <main className="max-w-7xl mx-auto px-6 md:px-16 mt-16 grid lg:grid-cols-12 gap-16">
        ...
        <aside className="lg:col-span-4 space-y-8 order-2 lg:order-1">
          ...
          <div className="p-6 border border-white/5 bg-zinc-950/50 space-y-6 sticky top-32">
            <h4 className="text-[10px] font-black text-[#006db1] uppercase tracking-[0.3em] border-b border-white/5 pb-4">
              Update_Metadata
            </h4>
            <div className="space-y-4">
              {[
                { label: "Source", value: update.source, color: "text-white" },
                { label: "Type", value: update.sourceType.replace("_", " "), color: "text-[#006db1]" },
                { label: "Published", value: update.date, color: "text-white" },
                { label: "Category", value: update.category, color: "text-[#006db1]" },
              ].map((item, i) => (
          ...
          <div className="space-y-4">
            ...
            {RELATED.map((rel, i) => (
              <Link 
                ...
                className="block p-4 bg-zinc-950/30 border border-white/5 hover:border-[#006db1]/30 transition-all group"
              >
                <span className="text-[8px] font-black text-[#006db1] uppercase tracking-widest">{rel.cat}</span>
                <h5 className="text-sm font-bold text-white uppercase italic mt-1 group-hover:text-[#006db1] transition-colors">
                  {rel.title}
                </h5>
              </Link>
            ))}
        ...
        <article className="lg:col-span-8 order-1 lg:order-2 space-y-8">
          ...
          {/* Contact CTA */}
          <div className="p-8 bg-zinc-950 border-2 border-[#006db1] mt-12">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
              ...
              <Link 
                href="/contact"
                className="flex items-center gap-4 bg-[#006db1] text-black px-6 py-3 text-[10px] font-black uppercase tracking-[0.3em] hover:bg-white transition-colors"
              >
          ...
          <div className="p-8 bg-zinc-900/30 border border-white/5">
            ...
            <div className="flex gap-4">
              <input 
                ...
                className="flex-1 bg-black border border-white/10 p-3 text-[10px] font-black uppercase tracking-widest focus:border-[#006db1] outline-none"
              />
              <button className="flex items-center justify-center gap-2 bg-white text-black px-6 py-3 text-[10px] font-black uppercase tracking-widest hover:bg-[#006db1] transition-colors">
      ...
      <footer className="mt-32 px-6 md:px-16 border-t border-white/5 pt-16">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          ...
          <Link href="/updates" className="text-[9px] font-black uppercase tracking-widest text-[#006db1] flex items-center gap-2 hover:text-white transition-colors">
